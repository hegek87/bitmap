...
int main(int argc, char **argv){
	...
	writeFile.write((char *)&iHeader.importantColors, 4);	// 4 bytes
									// 40 bytes written
	/*
	writeFile << iHeader.iHSize;			// 4 bytes
	writeFile << iHeader.width;			// 4 bytes
	writeFile << iHeader.height;			// 4 bytes
	writeFile << iHeader.colorPlanes;		// 2 bytes
	writeFile << iHeader.bPerPixel;		// 2 bytes
	writeFile << iHeader.compression;		// 4 bytes
	writeFile << iHeader.imageSize;		// 4 bytes
	writeFile << iHeader.horizRes;		// 4 bytes
	writeFile << iHeader.vertRes;			// 4 bytes
	writeFile << iHeader.colorPallette;		// 4 bytes
	writeFile << iHeader.importantColors;	// 4 bytes
		*/						// 40 bytes written	
	//12 bytes written
	...
	/*
	for(int i = 0; i < iHeader.width; ++i){
		for(int j = 0; i < iHeader.height; ++j){
			bool prime = isPrime(i*iHeader.width+j);
			if(prime){
				writeFile << RED;
			}
			else{
